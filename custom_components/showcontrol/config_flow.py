...
class ShowControlConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_test(
        ...
        """Show summary and run connection test on confirm."""
        errors: dict[str, str] = {}
        ...
        if user_input is not None:
            ...
            if not reachable:
                ...
                profile_storage = __import__("json").dumps(self._profile_data)
        ...
        entity_count = len(self._profile_data.get("entities", []))
        return self.async_show_form(
            step_id="test",
            data_schema=vol.Schema({}),
            errors=errors,
