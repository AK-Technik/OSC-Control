...
class ShowControlNumber(RestoreNumber):
    ...
    def __init__(
        ...
        self._osc_arg_template = config.get("osc_arg_template")  # e.g. "value" or "scaled"
    ...
    async def async_added_to_hass(self) -> None:
        """Restore previous state and register feedback listener."""
    ...
    def _build_args(self, value: float) -> list[Any]:
        template = self._osc_arg_template
        if template == "int":
            return [int(value)]
        if template == "scaled_255":
